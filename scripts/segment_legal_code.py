...
def extract_markdown_content(markdown_path: Path) -> str:
    ...
    try:
        ...
        raise ValueError(
            f"Failed to read markdown file {markdown_path}: {str(e)}"
        ) from e
...
def create_segments_dataframe(
    sections_df: pl.DataFrame, token_limit: int = 1024, words_per_token: float = 0.78
) -> pl.DataFrame:
    """
...
def segment_legal_code(
    ...
    try:
        if verbose:
            print(
                f"   Creating segments (token_limit={token_limit}, words_per_token={words_per_token})..."
            )

        segments_df = create_segments_dataframe(
            sections_df, token_limit=token_limit, words_per_token=words_per_token
        )
    ...
    print(
        f"   Sections:  {tables_dir / 'sections.parquet'} ({len(sections_df)} sections)"
    )
    print(
        f"   Segments:  {tables_dir / 'segments.parquet'} ({len(segments_df)} segments)"
    )
