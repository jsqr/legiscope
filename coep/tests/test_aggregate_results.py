...
_MODULE_PATH = PROJECT_ROOT / "coep" / "scripts" / "HPC_scripts" / "aggregate_results.py"
...
class TestAggregateResults:
    ...
    def test_collect_results_uses_latest_available_file_and_adds_source_metadata(
        ...
        pl.DataFrame(
            {"variable_name": ["dp_law"], "eval_score": [8]}
        ).write_csv(pa_dir / "benchmark_results_20260501_023513.csv")
        pl.DataFrame(
            {
                "jurisdiction_id": [""],
        ...
        assert (
            rows["PA-Philadelphia"]["_aggregate_source_timestamp"]
            == "20260501_023513"
        )
