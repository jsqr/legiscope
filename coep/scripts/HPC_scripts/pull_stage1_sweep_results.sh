#!/usr/bin/env bash
# pull_stage1_sweep_results.sh — Pull Stage 1 sweep aggregate artifacts from BigPurple.
...
usage() {
    cat <<'EOF'
Usage: pull_stage1_sweep_results.sh --netid NETID --sweep-id SWEEP_ID [options]

Pull Stage 1 sweep aggregate artifacts for one sweep identifier from BigPurple
...
Required:
  ...
  --sweep-id SWEEP_ID         Sweep prefix used in batch IDs, e.g. stage1_20260527_192246
...
Options:
  ...
  --open                      Open the downloaded stage1_batch_summary.md after download
  --dry-run                   Print commands and preview rsync actions
  -h, --help                  Show this help

Examples:
  ./coep/scripts/HPC_scripts/pull_stage1_sweep_results.sh \
    --netid tmh8501 \
    --sweep-id stage1_20260527_192246
...
remote_aggregate_dir_for_batch() {
    local batch_id="$1"
...
say "=== Pull Stage 1 Sweep Artifacts ==="
say "Remote       : ${REMOTE}"
...
for batch_id in "${BATCH_IDS[@]}"; do
    remote_batch_dir="$(remote_aggregate_dir_for_batch "$batch_id")" || die "could not resolve latest aggregate directory for batch ${batch_id}"
...
if [[ "$OPEN_AFTER" == true && "$DRY_RUN" == false ]]; then
    summary_md="${local_sweep_target}/stage1_batch_summary.md"
    summary_csv="${local_sweep_target}/stage1_batch_summary.csv"
    if [[ -f "$summary_md" ]]; then
        open_file "$summary_md"
    elif [[ -f "$summary_csv" ]]; then
        open_file "$summary_csv"
    else
        say "No stage1 batch summary file found to open."
    fi
fi
