#!/bin/bash
#SBATCH --job-name=qwen35-27b-test
...
#
# qwen3.5-27B_test.sh — Smoke-test vLLM serving Qwen/Qwen3.5-27B on 8x V100.
#
...
#
# Usage:
#   scp qwen3.5-27B_test.sh <netid>@bigpurple.nyumc.org:~/
#   ssh <netid>@bigpurple.nyumc.org
#   sbatch ~/qwen3.5-27B_test.sh
...
init_vllm_metrics_paths() {
    mkdir -p "$METRICS_DIR"
    VLLM_LOG_FILE="${METRICS_DIR}/qwen35_27b_test_${SLURM_JOB_ID}_vllm.log"
    GPU_MEM_LOG_FILE="${METRICS_DIR}/qwen35_27b_test_${SLURM_JOB_ID}_gpu.csv"
    GPU_PROC_LOG_FILE="${METRICS_DIR}/qwen35_27b_test_${SLURM_JOB_ID}_gpu_process.csv"
}
