#!/usr/bin/env bash
# pull_stage3_sweep_results.sh — Pull Stage 3 sweep aggregate artifacts from BigPurple.
...
usage() {
    cat <<'EOF'
Usage: pull_stage3_sweep_results.sh --netid NETID --sweep-id SWEEP_ID [options]

Pull Stage 3 sweep aggregate artifacts for one sweep identifier from BigPurple
...
Required:
  ...
  --sweep-id SWEEP_ID         Sweep prefix used in batch IDs, e.g. stage3_20260527_192246
...
Options:
  ...
  --open                      Open the downloaded stage3_batch_summary.md after download
...
say "=== Pull Stage 3 Sweep Artifacts ==="
say "Remote       : ${REMOTE}"
...
if [[ "$OPEN_AFTER" == true && "$DRY_RUN" == false ]]; then
    summary_md="${local_sweep_target}/stage3_batch_summary.md"
    summary_csv="${local_sweep_target}/stage3_batch_summary.csv"
    if [[ -f "$summary_md" ]]; then
        open_file "$summary_md"
    elif [[ -f "$summary_csv" ]]; then
        open_file "$summary_csv"
    else
        say "No stage3 batch summary file found to open."
    fi
fi
