...
def create_embeddings_df(
    ...
    if len(df) == 0:
        logger.warning("Empty DataFrame provided, returning with empty embeddings column")
        return df.with_columns(
            pl.lit([], dtype=pl.List(pl.Float64)).alias(embedding_col)
        )
    
    logger.debug(f"Processing {len(df)} rows for embedding generation")
    logger.debug(f"Using columns: heading='{heading_col}', text='{text_col}', embedding='{embedding_col}'")
    ...
    logger.debug(f"Concatenated {len(concatenated_texts)} texts for embedding generation")
